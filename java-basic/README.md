...
### 다형성(Polymorphism)
프로그래밍에서의 다형성은 한 객체가 여러 타입의 객체로 취급될 수 있는 능력.

#### compile error vs runtime error
컴파일 에러는 프로그램이 실행되기 전에 발생되는 오류로 잘못된 변수나 클래스 이름을 사용하면 IDE에서 알아차려 오류를 발생한다.  
런타임에러는 프로그램 실행 중에 발생되는 에러로 사용자가 프로그램을 사용하는 중에 에러가 발생하여 장애를 겪게되어 가장 피해야할 오류이다.

#### Java16, Pattern Matching for instanceof
자바 16부터 instanceof를 사용하며 동시에 변수 선언 가능  
다운캐스팅 코드를 생략할 수 있음

### Abstract
* 추상 메서드가 하나라도 존재하는 클래스는 추상 클래스여야만 한다.
  + 위 사항 위배시 컴파일 에러
  + 추상 메서드는 바디가 없다.
* 추상 메서드는 상속 받는 자식 클래스가 반드시 오버라이딩 해서 사용해야함.
  + 컴파일 에러
  + 바디를 만들면 컴파일 에러
  + 오버라이딩 설정이 안되면 자식 클래스도 추상 클래스가 되어야만 함.

### 순수 추상 클래스
* 모든 메서드가 추상 메서드인 추상 클래스
* 실행 로직이 전혀 없음, 주로 다형성만을 위함
* 하위 클래스는 모든 메서드를 구현해야함
* 순수 추상 클래스는 일종의 규격처럼 보여짐 자바는 이를 인터페이스라는 기능으로 제공하기도 함

### 인터페이스(구현, Implements)
* 순수 추상 클래스를 더욱 편리하게 사용할 수 있음
* 인터페이스 내에서 메서드를 생성할 때 public abstract 생략 가능(생략 권장)
* 다중 구현(다중 상속) 지원
* 멤버 변수 구현시 public static final이 모두 포함되어 있다고 간주해 생략 가능(생략 권장)
  + 상수와 같이 한 번 초기화한 값을 수정할 수 없음
* 하위 클래스에서 구현하지 않아도 되는 기본 메서드(바디가 존재하는 메서드)가 포함되는걸 방지함

> **인터페이스 내 메서드 접근제어자 주의사항**  
> 자바 8부터 등장한 default 키워드의 인터페이스 메서드는 사용이 가능하나 예외적이고 아주 특별한 경우에만 사용하여야 한다
> 자바 9의 private도 마찬가지

#### 인터페이스로 다중 구현을 지원하는 이유?
어차피 오버라이딩이 필수이기 때문
...
> > 디자인패턴 중 하나이다. 알고리즘을 클라이언트 코드의 변경 없이 쉽게 교체할 수 있다.
